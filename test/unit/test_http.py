...
These tests are designed to mock http responses from the udfs daemon. They
are used to determine if the functions in http.py are operating correctly.

Classes:
TestHttp -- A TCP client for interacting with an udfs daemon
"""
...
class TestHttp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = udfsapi.http.HTTPClient(
            'localhost',
            5001,
            'api/v0')
...
def test_stream_close(mocker):
    client = udfsapi.http.HTTPClient("localhost", 5001, "api/v0")
    mocker.patch("udfsapi.http._notify_stream_iter_closed")
    with HTTMock(return_okay):
        with client.request("/okay", stream=True) as response_iter:
            assert udfsapi.http._notify_stream_iter_closed.call_count == 0
        assert udfsapi.http._notify_stream_iter_closed.call_count == 1
        
        response_iter = client.request("/okay", stream=True)
        assert udfsapi.http._notify_stream_iter_closed.call_count == 1
        response_iter.close()
        assert udfsapi.http._notify_stream_iter_closed.call_count == 2
        
        client.request("/okay")
        assert udfsapi.http._notify_stream_iter_closed.call_count == 3
