# -*- coding: utf-8 -*-
"""udfs API Bindings for Python.

Classes:

 * Client – a TCP client for interacting with an udfs daemon
"""
...
DEFAULT_HOST = str(os.environ.get("PY_udfsAPI_DEFAULT_HOST", 'localhost'))
DEFAULT_PORT = int(os.environ.get("PY_udfsAPI_DEFAULT_PORT", 5001))
DEFAULT_BASE = str(os.environ.get("PY_udfsAPI_DEFAULT_BASE", 'api/v0'))
...
def assert_version(version, minimum=VERSION_MINIMUM, maximum=VERSION_MAXIMUM):
    ...
    Raises
    ------
    ~udfsapi.exceptions.VersionMismatch

    Parameters
    ----------
    version : str
        The version of an udfs daemon.
    minimum : str
        The minimal udfs version to allow.
    maximum : str
        The maximum udfs version to allow.
    """
...
def connect(host=DEFAULT_HOST, port=DEFAULT_PORT, base=DEFAULT_BASE,
    ...
    """Create a new :class:`~udfsapi.Client` instance and connect to the
    daemon to validate that its version is supported.

    Raises
    ------
    ~udfsapi.exceptions.VersionMismatch
    ~udfsapi.exceptions.ErrorResponse
    ~udfsapi.exceptions.ConnectionError
    ~udfsapi.exceptions.ProtocolError
    ~udfsapi.exceptions.StatusError
    ~udfsapi.exceptions.TimeoutError


    All parameters are identical to those passed to the constructor of the
    :class:`~udfsapi.Client` class.
...
class Client(object):
    """A TCP client for interacting with an udfs daemon.

    A :class:`~udfsapi.Client` instance will not actually establish a
    connection to the daemon until at least one of it's methods is called.

    Parameters
    ----------
    host : str
        Hostname or IP address of the computer running the ``udfs daemon``
        node (defaults to the local system)
    port : int
        The API port of the udfs deamon (usually 5001)
    base : str
    ...
    def __init__(self, host=DEFAULT_HOST, port=DEFAULT_PORT,
                 ...
                 **defaults):
        """Connects to the API port of an udfs node."""

        self.chunk_size = chunk_size

        self._client = self._clientfactory(host, port, base, **defaults)

    def push(self, files, recursive=False, pattern='**', *args, **kwargs):
        """Add a file, or directory of files to udfs.
        ...
        trickle : bool
            ...
            the dag; see `the FAQ <https://github.com/udfs/faq/issues/218>` for
        ...
        return self._client.request('/push', decoder='json',
    ...
    def get(self, multihash, **kwargs):
        """Downloads a file, or directory of files from udfs.
        ...
        ----------
        multihash : str
            The path to the udfs object(s) to be outputted
        """
        args = (multihash,)
    ...
    def cat(self, multihash, offset=0, length=-1, **kwargs):
        ...
        .. code-block:: python
            ...
            Traceback (most recent call last):
              ...
            udfsapi.exceptions.Error: this dag node is a directory
            >>> c.cat('QmeKozNssnkJ4NcyRidYgDY2jfRZqVEoRGfipkgath71bX')
            b'<!DOCTYPE html>\n<html>\n\n<head>\n<title>udfs example viewer</…'

        Parameters
        ----------
        multihash : str
            The path to the udfs object(s) to be retrieved
        offset : int
    ...
    def ls(self, multihash, **kwargs):
        ...
        ----------
        multihash : str
            The path to the udfs object(s) to list links from
    ...
    def block_put(self, file, **kwargs):
        """Stores the contents of the given file object as an udfs block.
        ...
        ----------
        file : io.RawIOBase
            The data to be stored as an udfs block

        Returns
        -------
            dict : Information about the new block

                   See :meth:`~udfsapi.Client.block_stat`
        """
    ...
    def object_data(self, multihash, **kwargs):
        r"""Returns the raw bytes in an udfs object.
    ...
    def object_new(self, template=None, **kwargs):
        """Creates a new object from an udfs template.
    ...
    def object_put(self, file, **kwargs):
        ...
        -------
            dict : Hash and links of the created DAG object
                   ...
                   See :meth:`~udfsapi.Object.object_links`
        """
        body, headers = multipart.stream_files(file, self.chunk_size)
    ...
    def object_patch_append_data(self, multihash, new_data, **kwargs):
        ...
        ----------
        multihash : str
            The hash of an udfs object to modify
    ...
    def object_patch_add_link(self, root, name, ref, create=False, **kwargs):
        ...
        ----------
        root : str
            udfs hash for the object being modified
        name : str
            name for the new link
        ref : str
            udfs hash for the object being linked to
        create : bool
    ...
    def object_patch_rm_link(self, root, link, **kwargs):
        ...
        ----------
        root : str
            udfs hash of the object to modify
        link : str
    ...
    def file_ls(self, multihash, **kwargs):
        ...
        that it is an ``/udfs/`` reference and not ``/ipns/``.
    ...
    def resolve(self, name, recursive=False, **kwargs):
        ...
        point at an udfs object, and DNS links can point at other DNS links,
        IPNS entries, or udfs objects. This command accepts any of these
        identifiers.

        .. code-block:: python

            >>> c.resolve("/udfs/QmTkzDwWqPbnAh5YiV5VwcTLnGdw … ca7D/Makefile")
            {'Path': '/udfs/Qmd2xkBfEwEs9oMTk77A6jrsgurpF3ugXSg7dtPNFkcNMV'}
            >>> c.resolve("/ipns/udfs.io")
            {'Path': '/udfs/QmTzQ1JRkWErjk39mryYw2WVaphAZNAREyMchXzYQ7c15n'}
        ...
        name : str
            The name to resolve
        recursive : bool
            Resolve until the result is an udfs name

        Returns
        -------
            dict : udfs path of resource
        """
    ...
    def name_publish(self, udfs_path, resolve=True, lifetime="24h", ttl=None,
                     key=None, **kwargs):
        ...
        .. code-block:: python
            ...
            >>> c.name_publish('/udfs/QmfZY61ukoQuCX8e5Pt7v8pRfhkyxwZK … GZ5d')
            {'Value': '/udfs/QmfZY61ukoQuCX8e5Pt7v8pRfhkyxwZKZMTodAtmvyGZ5d',
             'Name': 'QmVgNoP89mzpgEAAqK8owYoDEyB97MkcGvoWZir8otE9Uc'}

        Parameters
        ----------
        udfs_path : str
            udfs path of the object to be published
        resolve : bool
        ...
        key : string
             Name of the key to be used, as listed by 'udfs key list'.

        Returns
        -------
            dict : IPNS hash and the udfs path it points at
        """
    ...
    def name_resolve(self, name=None, recursive=False,
        ...
        .. code-block:: python
            ...
            >>> c.name_resolve()
            {'Path': '/udfs/QmfZY61ukoQuCX8e5Pt7v8pRfhkyxwZKZMTodAtmvyGZ5d'}
        ...
        recursive : bool
            Resolve until the result is not an udfs name (default: false)
        nocache : bool
            Do not use cached entries (default: false)

        Returns
        -------
            dict : The udfs path the IPNS hash points at
        """
        kwargs.setdefault("opts", {"recursive": recursive,
                                   "nocache": nocache})
    ...
    def dns(self, domain_name, recursive=False, **kwargs):
        ...
        can point to other DNS links, udfs objects, IPNS keys, etc.
        This command resolves those links to the referenced object.

        For example, with this DNS TXT record::

            >>> import dns.resolver
            >>> a = dns.resolver.query("udfs.io", "TXT")
            >>> a.response.answer[0].items[0].to_text()
            '"dnslink=/udfs/QmTzQ1JRkWErjk39mryYw2WVaphAZNAREyMchXzYQ7c15n"'
    ...
    def pin_add(self, path, *paths, **kwargs):
        ...
        Stores an udfs object(s) from a given path locally to disk.
        ...
        Returns
        -------
            dict : List of udfs objects that have been pinned
        """
        #PY2: No support for kw-only parameters after glob parameters
    ...
    def pin_rm(self, path, *paths, **kwargs):
        ...
        Returns
        -------
            dict : List of udfs objects that have been unpinned
        """
        #PY2: No support for kw-only parameters after glob parameters
        if "recursive" in kwargs:
    ...
    def pin_ls(self, type="all", **kwargs):
        ...
        Returns
        -------
            dict : Hashes of pinned udfs objects and why they are pinned
        """
    ...
    def pin_update(self, from_path, to_path, **kwargs):
        ...
        using first using :meth:`~udfsapi.Client.pin_add` to add a new pin
        for an object and then using :meth:`~udfsapi.Client.pin_rm` to remove
        ...
        .. code-block:: python
            ...
            {"Pins": ["/udfs/QmXMqez83NU77ifmcPs5CkNRTMQksBLkyfBf4H5g1NZ52P",
                      "/udfs/QmUykHAi1aSjMzHw3KmBoJjqRUQYNkFXm8K1y7ZsJxpfPH"]}
        ...
        Returns
        -------
            dict : List of udfs objects affected by the pinning operation
        """
        #PY2: No support for kw-only parameters after glob parameters
        if "unpin" in kwargs:
    ...
    def repo_gc(self, **kwargs):
        ...
        Returns
        -------
            dict : List of udfs objects that have been removed
        """
    ...
    def repo_stat(self, **kwargs):
        ...
        .. code-block:: python
            ...
            >>> c.repo_stat()
            {'NumObjects': 354,
             'RepoPath': '…/.local/share/udfs',
             'Version': 'fs-repo@4',
             'RepoSize': 13789310}

        Returns
        -------
            dict : General information about the udfs file repository
    ...
    def id(self, peer=None, **kwargs):
        ...
        .. code-block:: python
            ...
            'Addresses': [
                '/ip4/127.0.0.1/tcp/4001/udfs/QmVgNoP89mzpgEAAqK8owYo … E9Uc',
                '/ip4/10.1.0.172/tcp/4001/udfs/QmVgNoP89mzpgEAAqK8owY … E9Uc',
                '/ip4/172.18.0.1/tcp/4001/udfs/QmVgNoP89mzpgEAAqK8owY … E9Uc',
                '/ip6/::1/tcp/4001/udfs/QmVgNoP89mzpgEAAqK8owYoDEyB97 … E9Uc',
                '/ip6/fccc:7904:b05b:a579:957b:deef:f066:cad9/tcp/400 … E9Uc',
                '/ip6/fd56:1966:efd8::212/tcp/4001/udfs/QmVgNoP89mzpg … E9Uc',
                '/ip6/fd56:1966:efd8:0:def1:34d0:773:48f/tcp/4001/ipf … E9Uc',
                '/ip6/2001:db8:1::1/tcp/4001/udfs/QmVgNoP89mzpgEAAqK8 … E9Uc',
                '/ip4/77.116.233.54/tcp/4001/udfs/QmVgNoP89mzpgEAAqK8 … E9Uc',
                '/ip4/77.116.233.54/tcp/10842/udfs/QmVgNoP89mzpgEAAqK … E9Uc']}
    ...
    def bootstrap_list(self, **kwargs):
        ...
        .. code-block:: python
            ...
            >>> c.bootstrap_list()
            {'Peers': [
                '/ip4/104.131.131.82/tcp/4001/udfs/QmaCpDMGvV2BGHeYER … uvuJ',
                '/ip4/104.236.176.52/tcp/4001/udfs/QmSoLnSGccFuZQJzRa … ca9z',
                '/ip4/104.236.179.241/tcp/4001/udfs/QmSoLPppuBtQSGwKD … KrGM',
                …
                '/ip4/178.62.61.185/tcp/4001/udfs/QmSoLMeWqB7YGVLJN3p … QBU3']}
    ...
    def bootstrap_add(self, peer, *peers, **kwargs):
        ...
        ----------
        peer : str
            udfs MultiAddr of a peer to add to the list
    ...
    def bootstrap_rm(self, peer, *peers, **kwargs):
        ...
        ----------
        peer : str
            udfs MultiAddr of a peer to remove from the list
    ...
    def swarm_peers(self, **kwargs):
        ...
        .. code-block:: python
            ...
            >>> c.swarm_peers()
            {'Strings': [
                '/ip4/101.201.40.124/tcp/40001/udfs/QmZDYAhmMDtnoC6XZ … kPZc',
                '/ip4/104.131.131.82/tcp/4001/udfs/QmaCpDMGvV2BGHeYER … uvuJ',
                '/ip4/104.223.59.174/tcp/4001/udfs/QmeWdgoZezpdHz1PX8 … 1jB6',
                …
                '/ip6/fce3: … :f140/tcp/43901/udfs/QmSoLnSGccFuZQJzRa … ca9z']}
    ...
    def swarm_connect(self, address, *addresses, **kwargs):
        ...
        .. code-block:: python
            ...
            >>> c.swarm_connect("/ip4/104.131.131.82/tcp/4001/udfs/Qma … uvuJ")
    ...
    def swarm_disconnect(self, address, *addresses, **kwargs):
        ...
        The disconnect is not permanent; if udfs needs to talk to that address
        later, it will reconnect.

        .. code-block:: python

            >>> c.swarm_disconnect("/ip4/104.131.131.82/tcp/4001/udfs/Qm … uJ")
    ...
    def dht_query(self, peer_id, *peer_ids, **kwargs):
        ...
        .. code-block:: python
            ...
            >>> c.dht_query("/ip4/104.131.131.82/tcp/4001/udfs/QmaCpDM … uvuJ")
    ...
    def dht_put(self, key, value, **kwargs):
        ...
        You may only use keytypes that are supported in your ``udfs`` binary:
        ``go-udfs`` currently only supports the ``/ipns/`` keytype. Unless you
        have a relatively deep understanding of the key's internal structure,
        you likely want to be using the :meth:`~udfsapi.Client.name_publish`
        instead.
    ...
    def config_show(self, **kwargs):
        ...
        Returns
        -------
            dict : The entire udfs daemon configuration
        """
    ...
    def log_ls(self, **kwargs):
        ...
        .. code-block:: python
            ...
            >>> c.log_ls()
            {'Strings': [
                'github.com/udfs/go-libp2p/p2p/host', 'net/identify',
                ...
                'commands/http', 'cmd/udfs', 'command', 'conn', 'gc',
                'peerstore', 'core', 'coreunix', 'fsrepo', 'core/server',
                'boguskey', 'github.com/udfs/go-libp2p/p2p/host/routed',
                'diagnostics', 'namesys', 'fuse/udfs', 'node', 'secio',
                'core/commands', 'supernode', 'mdns', 'path', 'table',
                'swarm2', 'peerqueue', 'mount', 'fuse/ipns', 'blockstore',
                'github.com/udfs/go-libp2p/p2p/host/basic', 'lock', 'nat',
                'importer', 'corerepo', 'dht.pb', 'pin', 'bitswap_network',
                'github.com/udfs/go-libp2p/p2p/protocol/relay', 'peer',
                'transport', 'dht', 'offlinerouting', 'tarfmt', 'eventlog',
                'udfsaddr', 'github.com/udfs/go-libp2p/p2p/net/swarm/addr',
    ...
    def files_cp(self, source, dest, **kwargs):
        ...
        Due to the nature of udfs this will not actually involve any of the
    ...
    def shutdown(self):
        """Stop the connected udfs daemon instance.

        Sending any further requests after this will fail with
        ``udfsapi.exceptions.ConnectionError``, until you start another udfs
        daemon instance.
        """
        try:
    ...
    def add_bytes(self, data, **kwargs):
        """Adds a set of bytes as a file to udfs.
        ...
        Returns
        -------
            str : Hash of the added udfs object
        """
    ...
    def add_str(self, string, **kwargs):
        """Adds a Python string as a file to udfs.
    ...
    def add_json(self, json_obj, **kwargs):
        """Adds a json-serializable Python dict as a json file to udfs.
    ...
    def get_json(self, multihash, **kwargs):
        ...
        ----------
        multihash : str
           Multihash of the udfs object to load

        Returns
        -------
            object : Deserialized udfs JSON object value
        """
        return self.cat(multihash, decoder='json', **kwargs)

    def add_pyobj(self, py_obj, **kwargs):
        """Adds a picklable Python object as a file to udfs.

        .. deprecated:: 0.4.2
           The ``*_pyobj`` APIs allow for arbitrary code execution if abused.
           Either switch to :meth:`~udfsapi.Client.add_json` or use
           ``client.add_bytes(pickle.dumps(py_obj))`` instead.

        Please see :meth:`~udfsapi.Client.get_pyobj` for the
    ...
    def get_pyobj(self, multihash, **kwargs):
        """Loads a pickled Python object from udfs.

        .. deprecated:: 0.4.2
           The ``*_pyobj`` APIs allow for arbitrary code execution if abused.
           Either switch to :meth:`~udfsapi.Client.get_json` or use
        ...
        Returns
        -------
            object : Deserialized udfs Python object
        """
        warnings.warn("Using `*_pyobj` on untrusted data is a security risk",
                      DeprecationWarning)
    ...
    def pubsub_peers(self, topic=None, **kwargs):
        ...
        Lists the id's of other udfs users who we
        are connected to via some topic. Without specifying
        a topic, udfs peers from all subscribed topics
        will be returned in the data. If a topic is specified
        only the udfs id's of the peers from the specified
        ...
        Returns
        -------
            dict : Dictionary with the ke "Strings" who's value is id of udfs
    ...
    def pubsub_sub(self, topic, discover=False, **kwargs):
        ...
        Subscribing to a topic in udfs means anytime
