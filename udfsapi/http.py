...
This is pluggable into the udfs Api client and will hopefully be supplemented
...
class HTTPClient(object):
    """An HTTP client for interacting with the udfs daemon.

    Parameters
    ----------
    host : str
        The host the udfs daemon is running on
    port : int
        The port the udfs daemon is running at
    base : str
        The path prefix for API calls
    defaults : dict
        The default parameters to be passed to
        :meth:`~udfsapi.http.HTTPClient.request`
    """
    ...
    def request(self, path,
        ...
        """Makes an HTTP request to the udfs daemon.

        This function returns the contents of the HTTP response from the udfs
        daemon.
    ...
    def download(self, path, args=[], filepath=None, opts={},
        ...
        """Makes a request to the udfs daemon to download a file.

        Downloads a file or files from udfs into the current working
        ...
        path : str
            The REST command path to send
        filepath : str
            The local path where udfs will store downloaded files
